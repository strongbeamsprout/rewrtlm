test_that("lm.rewrt works", {
  ...
  expect_equal(summary.lm.rewrt(lm.rewrt(mpg ~ cyl + hp, data = mtcars), correlation = TRUE, prt = FALSE)$correlation,
  ...
  expect_equal(summary.lm.rewrt(lm.rewrt(dist ~ speed, data = cars), correlation = TRUE, prt = FALSE)$fstatistic,
